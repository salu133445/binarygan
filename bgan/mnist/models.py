...
from bgan.mnist.components import End2EndGenerator, Discriminator, Generator
from bgan.utils import image_io
...
class Model(object):
        ...
        if (self.config['gan']['type'] == 'wgan'
            ...
            if self.config['gan']['type'] == 'wgan-gp':
                eps = tf.random_uniform(
                    [tf.shape(self.x_)[0], 1, 1, 1], 0.0, 1.0)
    ...
    def save_samples(self, filename, samples, shape=None, postfix=None):
        """Save samples to an image file."""
        if shape is None:
    ...
    def run_sampler(self, targets, feed_dict, postfix=None):
        ...
        self.save_samples(filename, samples, shape)
...
class End2EndGAN(Model):
    ...
    def build(self):
        ...
        data_shape = (self.config['batch_size'], self.config['out_height'],
                      self.config['out_width'], self.config['out_channel'])
    ...
    def train(self, x_train, train_config):
        ...
        # Save samples
        self.save_samples('x_train', x_train)
        self.save_samples('x_sample', self.x_sample)
        ...
        for epoch in range(train_config['num_epoch']):
            ...
            for batch in range(num_batch):
                ...
                if train_config['sample_along_training']:
                    if counter%500 == 0 or (counter < 300 and counter%100 == 0):
